...
`chatqiniu --version` prints the package version. Chatqiniu uses the shared `chatstyle.add_tree_option()` to render the real Click registry: `chatqiniu --tree` includes parameter signatures, while `chatqiniu --tree-brief` preserves the same nodes and purpose descriptions without signatures.
